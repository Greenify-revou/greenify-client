...
const ManageProducts = () => {
    ...
    const fetchProducts = async () => {
        ...
        try {
            const response = await fetch(
                "http://127.0.0.1:5000/api/v1/sellers/get_all_seller_products",
                {
                    method: "GET",
                    headers: {
                        "Content-Type": "application/json",
                        Authorization: `Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..JTQSBdChdu3CYDbgJTsIJ51_gwkkPww6c8FebReBNLw`, // Replace with your token
                    },
                }
            );
            ...
            Swal.fire("Error", error.message || "Failed to fetch products", "error");
        } finally {
    ...
    const fetchCategories = async () => {
        try {
            const response = await fetch("http://127.0.0.1:5000/api/v1/products/get-all-category", {
                method: "GET",
                // headers: {
                //     Authorization: `Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..qrpKoBv7AZbPW-FtD4E36PGcXho_hkvc1QPQsl3Z9vI`, // Replace with your token
                // },
            });
            const data = await response.json();
            if (data.status === "success" && data.data) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const url = editingId
                ? `http://127.0.0.1:5000/api/v1/products/${editingId}` // Update endpoint
                : "http://127.0.0.1:5000/api/v1/products/add-product"; // Add endpoint
    ...
    const columns = [
        ...
        { name: "Name", selector: (row: Product) => row.product_name },
        { name: "Category", selector: (row: Product) => row.category_name },
        {
            name: "Discount %",
            selector: (row: Product) => `${row.discount}%`,
        },
        ...
        {
            name: "Recycle %",
            selector: (row: Product) => `${row.recycle_material}%`,
        },
        {
            name: "Actions",
            cell: (row: Product) => (
